...
def moneymaker_clay(Take_Human_Break=False, Run_Duration_hours=4, color=6):
    ...
    while time.time() < t_end:
        ...
        if mined_text.strip().lower() != 'mining' and mined_text.strip().lower() != 'mininq':
            #actions = 'Not mining'
            spot = functions.find_Object(color, x_win, y_win, x_win+w_win, y_win+h_win)
            if Take_Human_Break:
...
if __name__ == "__main__":
    ...
    moneymaker_clay(Take_Human_Break=False, Run_Duration_hours=Run_Duration_hours, color=red)
